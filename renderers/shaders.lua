local shaders = {
      bloom = [[
         ...
         extern number camera_x = 0;
         extern number camera_y = 0;
         ...
         {
            vec2 tex_coords_camera =   vec2( 
                                          max(min(camera_x/canvas_w, 0),1),
                                          max(min(1-camera_y/canvas_h,0),1) 
                                       );
            vec4 texcolor = Texel(texture, texture_coords - tex_coords_camera);
      ...
      crt = [[
         ...
         vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 pixel_coords)
         {
            ras=max(0.,sign(-.5+rand(vec2(1.3456,.3573)*floor(30.+iGlobalTime*20.))));
            ...
            return vec4(0.0,0.0,0.0,0.0)+ra*.03+(ral*.1+ra*.1)*rab;
         }
      ]]
   }
return shaders